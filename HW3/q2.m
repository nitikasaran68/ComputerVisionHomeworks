...
dat = importdata('/Users/nitikasaran/Downloads/tracking_video_dataset/3/groundtruth_rect.txt')
...
points = detectHarrisFeatures(rgb2gray(videoFrame), 'ROI', bbox);
...
while ~isDone(videoFileReader)
    ...
    if size(visiblePoints, 1) >= 2 % need at least 2 points
        ...
        finbbox = [min(bboxPoints(:,1)) min(bboxPoints(:,2)) (max(bboxPoints(:,1))-min(bboxPoints(:,1))) ( max(bboxPoints(:,2))- min(bboxPoints(:,2)))];
        
        % Insert a bounding box around the object being tracked
        bboxPolygon = reshape(bboxPoints', 1, []);
        videoFrame = insertShape(videoFrame, 'Polygon', bboxPolygon, ...
            'LineWidth', 2);
        error = [error; bboxOverlapRatio(currentbbox,finbbox)];            
